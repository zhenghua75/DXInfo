...
{
	/// <summary>
	/// wfmDeptStorageEnterAdd 的摘要说明。
	/// </summary>
	public class wfmDeptStorageEnterAdd : wfmBase
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if(!IsPostBack)
			{
				...
				if(strRdId==""||strRdId==null)
				{
					...
					this.txtEnterCode.Text="ASE"+strCycle+"----";
		...
		private void btAdd_Click(object sender, System.EventArgs e)
		{
			...
			ol.cnvcOperType = "添加分货入库主单";
			ol.cnvcOperID = this.oper.strLoginID;
			ol.cnvcDeptID = this.oper.strDeptID;

			StorageFacade sto = new StorageFacade();				
			int ret = sto.AddRdRecordCom("RD007",ol,rd);
			if(ret > 0 )
			{
				this.Popup("添加分货入库单内容成功！");
			}
			else
			{
				this.Popup("添加分货入库单内容失败！");
			}
		}
		...
		private void btMod_Click(object sender, System.EventArgs e)
		{
			...
			{
				this.Popup("分货入库单主表标识不正确！");
				return;
			}

			Entity.OperLog ol = new AMSApp.zhenghua.Entity.OperLog();
			ol.cnvcOperType = "修改分货入库主单";
			ol.cnvcOperID = this.oper.strLoginID;
			ol.cnvcDeptID = this.oper.strDeptID;
			...
			if(ret > 0 )
			{
				this.Popup("修改分货入库单内容成功！");
			}
			else
			{
				this.Popup("修改分货入库单内容失败！");
			}
		}
