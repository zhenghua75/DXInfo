...
{
	/// <summary>
	/// wfmPoStockReturn 的摘要说明。
	/// </summary>
	public class wfmPoStockReturn : wfmBase
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack )
			{
				...
				this.ddlState.Items.Add(new ListItem("未出库","0"));
				this.ddlState.Items.Add(new ListItem("已出库","1"));
		...
		private void btnQuery_Click(object sender, System.EventArgs e)
		{
			...
			string strsql="select cnnRdID as 主表标识,cnvcCode as 采购退货单号,cnvcDepID as 部门,cnvcWhCode as 仓库,cnvcMaker as 制单人,cndARVDate as 退货日期,";
			strsql+="(case cnvcState when '0' then '未出库' else '已出库' end) as 状态 from tbRdRecord where cnvcRdCode='RD002'";
			strsql+=" and cndARVDate between '"+strBeginDate+"' and '"+strEndDate+" 23:59:59'";
			if(this.txtPoEnterID.Text.Trim()!="")
				strsql+=" and cnvcCode='"+this.txtPoEnterID.Text.Trim()+"'";
			...
			{
				dtout.Rows[i]["单据内容"]="<a href='wfmPoStockReturnAdd.aspx?rdid=" + dtout.Rows[i]["主表标识"].ToString() +"'>内容</a>";
			}
			dtout.Columns.Add("单据明细");
			for(int i=0;i<dtout.Rows.Count;i++)
			{
				dtout.Rows[i]["单据明细"]="<a href='wfmPoStockReturnDetail.aspx?rdid=" + dtout.Rows[i]["主表标识"].ToString()+"&code="+dtout.Rows[i]["采购退货单号"].ToString()+"&dept="+dtout.Rows[i]["部门"].ToString()+"&whid="+dtout.Rows[i]["仓库"].ToString()+"'>明细</a>";
			}
			this.TableConvert(dtout,"部门","NewDept","vcCommCode","vcCommName");
			this.TableConvert(dtout,"仓库","Warehouse","vcCommCode","vcCommName");
			dtout.TableName="采购出库单列表";
			Session["QUERY"] = dtout;
		...
		{
			Response.Redirect("wfmPoStockReturnAdd.aspx");
		}
