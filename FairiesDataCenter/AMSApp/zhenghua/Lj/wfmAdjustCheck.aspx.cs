...
{
	/// <summary>
	/// wfmAdjustCheck 的摘要说明。
	/// </summary>
	public class wfmAdjustCheck : wfmBase
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if(!this.IsPostBack)
			{
				...
				if(Session["tbCheckSerial_Query"] != null)
				{
					Session.Remove("tbCheckSerial_Query");
				}
				BindGrid();
			}
		}
		...
		public DataTable GetProduct()
		{
			...
			if(Session["tbCheckSerial_Query"] != null)
			{
				dtProduct = (DataTable) Session["tbCheckSerial_Query"];
			}
			else
			{
				dtProduct = Helper.Query("select cnnSerialNo,cnvcName,cnvcCode,cnnCount+cnnAddCount-cnnReduceCount as cnnSum,cnnCount,cnnAddCount,cnnReduceCount,convert(char(10),cndCreateDate,121) as cndCreateDate,cnbIsSales from tbCheckSerial where 1<>1");
				Session["tbCheckSerial_Query"] = dtProduct;
			}
			return dtProduct;
		}
		...
		{
			//查询
			string strSql = "select cnnSerialNo,cnvcName,cnvcCode,cnnCount+cnnAddCount-cnnReduceCount as cnnSum,cnnCount,cnnAddCount,cnnReduceCount,convert(char(10),cndCreateDate,121) as cndCreateDate,cnbIsSales from tbCheckSerial where convert(char(10),cndCreateDate,121) >= '"+txtBeginDate.Text+"' and  "
		...
		private void MyDataGrid_ItemDataBound(object sender, System.Web.UI.WebControls.DataGridItemEventArgs e)
		{
			if(e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
			{
				...
				delBttn.Attributes.Add("onclick",@"javascript:return confirm('确定删除盘点序号为：【" + e.Item.Cells[0].Text+"】产品名称为：【"+e.Item.Cells[2].Text + "】的产品吗?');"); 
			}
		}
