IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[sp_DXInfo_GetCurrentStockOfCheck]') AND type in (N'P'))
drop procedure dbo.sp_DXInfo_GetDeptInventory
IF  not EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[sp_DXInfo_GetCurrentStockOfCheck]') AND type in (N'P'))
begin
exec('create procedure [dbo].[sp_DXInfo_GetCurrentStockOfCheck]
@WhId uniqueidentifier
as
begin
select * from CurrentStock e
left join
...
join
(select InvId,Batch from CurrentStock where WhId=@WhId and Num=0 
