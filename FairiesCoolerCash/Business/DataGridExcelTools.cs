...
namespace FairiesCoolerCash.Business
{
    public static class DataGridExcelTools
    {
        ...
        public static void ExportToExcel(this DataGrid grid, IEnumerator itemsSource)
        {
            Thread thread = new Thread(StartExport);
            var list = new List<object>();
            while (itemsSource.MoveNext())
            {
                list.Add(itemsSource.Current);
            }
            thread.Start(PrepareData(grid, list));
        }
        public static void ExportToExcel(this DataGrid grid, IEnumerable itemsSource)
        {
            Thread thread = new Thread(StartExport);
            thread.Start(PrepareData(grid, itemsSource));
        }
        ...
        {
            return PrepareData(grid, grid.ItemsSource);
        }
        private static object[,] PrepareData(DataGrid grid, IEnumerable itemsSource)
        {
